...
export const signInWithGoogle = async () => {
  try {
    ...
    const userRole = isAdminEmail(result.user.email) ? 'admin' : 'user';
    ...
    try {
      ...
      if (response.ok) {
        ...
        const finalRole = isAdminEmail(result.user.email) ? 'admin' : (user.role || 'user');
        ...
        if (token) {
          ...
          return { 
            user: result.user, 
            appToken: token, 
            backendUser: { ...user, role: finalRole },
    ...
    // Fallback: Save Firebase token directly with admin check
    const userData = {
      email: result.user.email,
      displayName: result.user.displayName,
      uid: result.user.uid,
